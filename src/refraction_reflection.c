...
/*   Updated: 2017/01/18 19:16:40 by gpinchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
VEC3	compute_refraction(ENGINE *e, CAST_RETURN *re, RAY *r, float a)
{
	...
	mtl = re->mtl;
	if (mtl.alpha >= 1 || e->refr_iteration >= e->max_refr)
		return ((VEC3){0, 0, 0});
	e->refr_iteration++;
	c = (VEC3){0, 0, 0};
	d = vec3_refract(r->direction, re->intersect.normal, mtl.refraction, a);
	...
	{
		get_ret_mtl(&cr);
		c = vec3_add(c, compute_refraction(e, &cr, &ray, mtl.refraction));
		c = vec3_add(c, compute_lighting(e, &cr));
		c = vec3_interp(interp_linear, c, (VEC3){0, 0, 0}, mtl.alpha);
		c = vec3_interp(interp_linear, c, (VEC3){0, 0, 0}, mtl.roughness);
		c = vec3_mult(c, mtl.refraction_color);
	}
	e->refr_iteration = 0;
	return (c);
}
