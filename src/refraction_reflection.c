...
/*   Updated: 2016/12/05 12:55:44 by gpinchon         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
VEC3	compute_refraction(ENGINE *engine, CAST_RETURN ret, RAY cur_ray, float aior)
{
	...
	if ((refrret = cast_ray(engine, engine->active_scene, ray)).intersect.intersects)
	{
		...
		//color = vec3_add(color, vec3_scale(ret.mtl.refraction_color, refraction_medium(cur_ray.direction, ret.intersect.normal, ret.mtl.refraction, aior)));
...
/*VEC3	blur_reflection(ENGINE *engine, CAST_RETURN ret, RAY cur_ray)
{
	...
	while (sample <= 4)
	{
		ray.direction = vec3_normalize(vec3_fadd(vec3_reflect(cur_ray.direction, ret.intersect.normal), frand_a_b(-ret.mtl.roughness, ret.mtl.roughness) * 0.05f));
